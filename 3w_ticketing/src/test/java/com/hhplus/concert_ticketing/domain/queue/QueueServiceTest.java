...
class QueueServiceTest {
    ...
    void 토큰_조회_상태에_따른_반환_테스트() {
        ...
        ResponseEntity<TokenResponse> responseWaiting = queueService.checkToken(waitingToken);
        assertEquals(HttpStatus.CONTINUE, responseWaiting.getStatusCode());
        assertEquals("100", responseWaiting.getBody().getResult());
        ...
        ResponseEntity<TokenResponse> responseActivate = queueService.checkToken(activateToken);
        assertEquals(HttpStatus.SEE_OTHER, responseActivate.getStatusCode());
        assertEquals("/reservation-page", responseActivate.getHeaders().getLocation().toString());
